import { ATLASSIAN_CLIENT_ID, ATLASSIAN_CLIENT_SECRET, ATLASSIAN_REDIRECT_URI } from "./constants"
...
export async function handleCallback(code: string, returnedState: string | null): Promise<void> {
  ...
  if (!savedState || returnedState !== savedState) throw new Error("Invalid OAuth state — possible CSRF. Please try logging in again.")
...
export async function getValidToken(): Promise<{ token: string; cloudId: string }> {
  const stored = getStoredTokens()
