...
export async function getEpics(): Promise<JiraEpic[]> {
  ...
  return data.issues as JiraEpic[]
}

export async function getTasksForEpics(epicKeys: string[]): Promise<JiraIssue[]> {
  ...
  const res = await jiraFetch("/search/jql", {
    ...
    body: JSON.stringify({
      jql,
      fields: ["summary", "description", "status", "issuetype", "assignee", "parent", "priority", "duedate"],
      maxResults: 500,
    }),
  })
...
export async function updateIssue(
  ...
  if (fields.description !== undefined) updateFields.description = toAdf(fields.description)
  if (fields.issuetype !== undefined) updateFields.issuetype = { name: fields.issuetype }
  if (fields.epicKey !== undefined) updateFields.parent = { key: fields.epicKey }
  if ("assigneeAccountId" in fields) {
    updateFields.assignee = fields.assigneeAccountId ? { accountId: fields.assigneeAccountId } : null
  }
...
export async function applyTransition(issueKey: string, transitionId: string): Promise<void> {
  const res = await jiraFetch(`/issue/${issueKey}/transitions`, {
    method: "POST",
...
export async function transitionToStatus(issueKey: string, targetStatusNames: string[]): Promise<void> {
  const transitions = await getTransitions(issueKey)
  const match = transitions.find((t) =>
    targetStatusNames.some((s) => t.to.name.toLowerCase() === s.toLowerCase())
  )
  if (!match) throw new Error(`No transition found for statuses: ${targetStatusNames.join(", ")}`)
  await applyTransition(issueKey, match.id)
}

export async function getAssignableUsers(): Promise<JiraUser[]> {
  const res = await jiraFetch(`/user/assignable/search?project=${JIRA_PROJECT_KEY}&maxResults=50`)
  if (!res.ok) return []
